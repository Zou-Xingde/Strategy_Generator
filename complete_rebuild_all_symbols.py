...
def backup_database():
    ...
    backup_path = f"database/backup_complete_rebuild_{timestamp}.duckdb"
...
def setup_database_schema(conn):
    ...
    # 清空舊的測試資料
    conn.execute("DELETE FROM candlestick_data_new WHERE data_source = 'rebuild_test'")
...
def process_symbol_batch(symbol, tick_file, conn, max_chunks=None):
    """分批處理單個商品的資料"""
    print(f"\n{'='*60}")
    ...
    # 清空該商品的舊資料
    conn.execute("DELETE FROM candlestick_data_new WHERE symbol = ? AND data_source = 'complete_rebuild'", [symbol])
    ...
    try:
        ...
        for chunk in chunk_iter:
            ...
            if max_chunks and chunk_num >= max_chunks:
                print(f"⚠️ 已達到最大批次數 {max_chunks}，停止處理")
                break
...
def verify_final_results(conn):
    ...
    for symbol, config in SYMBOLS_CONFIG.items():
        ...
        for tf_code in TIMEFRAMES.keys():
            result = conn.execute("""
                ...
                WHERE symbol = ? AND timeframe = ? AND data_source = 'complete_rebuild'
    ...
    result = conn.execute("""
        SELECT COUNT(*) FROM candlestick_data_new 
        WHERE data_source = 'complete_rebuild'
    """).fetchone()
