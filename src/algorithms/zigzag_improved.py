...
針對長期數據優化的ZigZag演算法，能夠更好地識別重要的轉折點
"""
...
class ImprovedZigZagAlgorithm(BaseAlgorithm):
    """改進的ZigZag波段識別演算法"""
    ...
    def calculate(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"改進ZigZag計算完成，找到 {len(zigzag_points)} 個轉折點")
            
            return result_df
            
        except Exception as e:
            logger.error(f"改進ZigZag計算失敗: {e}")
            raise
