...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('zigzag_processing.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ZigZagProcessor:
    """ZigZag波段處理器"""
    
    def __init__(self):
        self.db_path = DUCKDB_PATH
        # 使用更適合長期數據的參數：5%偏差，12個bar深度
        self.algorithm = ZigZagAlgorithm(deviation=5.0, depth=12)
        self.symbols = ['XAUUSD', 'US30', 'US100']
        self.timeframes = list(TIMEFRAMES.keys())
        self.next_id = 1  # 手動管理ID
        self.batch_size = 100000  # 分批處理大小
    ...
    def _calculate_zigzag_swings_single(self, df, symbol, timeframe):
        ...
        for idx, row in zigzag_data.iterrows():
            record = {
                ...
                'version_hash': 'c105ff985a9cee2',  # 版本雜湊
    ...
    def show_statistics(self, conn):
        ...
        try:
            ...
            for symbol, timeframe, count in detail_stats:
                logger.info(f"  {symbol} {timeframe}: {count} 筆")
...
def main():
    """主函數"""
    logger.info("=== ZigZag波段計算和匯入資料庫 ===")
    
    processor = ZigZagProcessor()
    
    try:
        processor.process_all_symbols()
        logger.info("ZigZag波段處理完成！")
        
    except Exception as e:
        logger.error(f"ZigZag波段處理失敗: {e}")
        sys.exit(1)
