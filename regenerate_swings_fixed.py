...
"""
使用修復版本算法重新生成完整的波段數據
解決SettingWithCopyWarning和切片問題，處理所有歷史數據
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('regenerate_swings_fixed.log'),
...
class FixedSwingRegenerator:
    ...
    def __init__(self):
        ...
        from zigzag_improved_fixed import ImprovedZigZagAlgorithmFixed
        self.algorithm = ImprovedZigZagAlgorithmFixed(deviation=2.0, depth=15, min_swing_bars=3)
    ...
    def backup_swing_data(self, conn):
        ...
        try:
            ...
            if count > 0:
                # 創建備份表
                backup_table = f"swing_data_backup_fixed_{int(time.time())}"
    ...
    def calculate_zigzag_swings(self, df, symbol, timeframe):
        """計算ZigZag波段（使用修復版本算法）"""
        try:
            logger.info(f"開始計算 {symbol} {timeframe} 的ZigZag波段（修復版本）...")
        ...
        try:
            ...
            for idx, row in zigzag_data.iterrows():
                record = {
                    ...
                    'symbol': symbol,
                    'timeframe': timeframe,
                    'algorithm_name': 'zigzag_fixed',
                    'version_hash': 'fixed_regeneration_v1',  # 新的版本雜湊
            ...
            logger.error(f"單次計算 {symbol} {timeframe} 失敗: {e}")
            return None
    ...
    def process_all_symbols(self):
        """處理所有品種的ZigZag波段（修復版本）"""
        logger.info("開始使用修復版本算法重新生成所有品種的ZigZag波段...")
...
def main():
    """主函數"""
    logger.info("=== 開始使用修復版本算法重新生成完整波段數據 ===")
    
    regenerator = FixedSwingRegenerator()
    
    try:
        regenerator.process_all_symbols()
        logger.info("=== 波段數據重新生成完成（修復版本） ===")
    except Exception as e:
